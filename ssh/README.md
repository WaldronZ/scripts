...
```bash
mkdir -p ~/scripts/ssh
cp ~/scripts-repo/ssh/setup_ssh_key.sh ~/scripts/ssh/setup_ssh_key.sh
chmod +x ~/scripts/ssh/setup_ssh_key.sh
```
...
ssh() {
  if [[ $# -eq 0 ]]; then
    bash ~/scripts/ssh/setup_ssh_key.sh --login
  else
    command ssh "$@"
  fi
}
...
sshkey() {
  bash ~/scripts/ssh/setup_ssh_key.sh --setup "$@"
}

# Create/update an inner Host through an existing jump Host.
sshkeyjump() {
  bash ~/scripts/ssh/setup_ssh_key.sh --jump "$@"
}
```
